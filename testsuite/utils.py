...
def load_ggi_network(ggi_network_selector, expression_data):
    ...
    if str(ggi_network_selector) == "CUSTOM":
        edges = pd.read_csv("../data/networks/CUSTOM.csv", sep="\t")  # , header=None)
        edges.columns = [s.strip() for s in edges.columns]
        edges = edges[["gene_ID_1", "gene_ID_2"]]
        edges = edges.values
        G = nx.Graph()
    ...
    ggi_network = nx.read_graphml(
        f"../data/networks/{str(ggi_network_selector)}.graphml", node_type=int
    )
    gene_ids = nx.get_node_attributes(ggi_network, "GeneID")
    selected_genes = set([str(x) for x in expression_data.columns])
    selected_nodes = [
        node for node in ggi_network.nodes() if gene_ids[node] in selected_genes
    ]
...
def load_expression_data(condition_selector):
    ...
    expression_data = pd.read_csv(
        f"../data/expression/{str(condition_selector)}/expr.csv.zip", index_col=0
    )
...
def compute_gene_p_values(expression_data, phenotypes):
    ...
    return {
        gene: sps.mannwhitneyu(cases[gene], controls[gene], alternative="two-sided")[1]
        for gene in genes
    }
...
def compute_seed_statistics(ggi_network, seed_genes):
    ...
    gene_ids = nx.get_node_attributes(ggi_network, "GeneID")
    seed_nodes = [
        node for node in ggi_network.nodes() if gene_ids[node] in set(seed_genes)
    ]
    subgraph = ggi_network.subgraph(seed_nodes)
    lcc_ratio = (
        np.max([len(comp) for comp in nx.connected_components(subgraph)])
    ...
    for source, target in itt.combinations(seed_nodes, 2):
        try:
            sum_shortest_distances += nx.shortest_path_length(
                ggi_network, source=source, target=target
            )
            num_combinations += 1
