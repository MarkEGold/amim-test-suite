...
def generate_rewired_network(ggi_network, seed):
    ...
    rewired_network = nx.expected_degree_graph(
        degree_sequence, seed=seed, selfloops=False
    )
    gene_ids = nx.get_node_attributes(ggi_network, "GeneID")
    nx.set_node_attributes(rewired_network, gene_ids, "GeneID")
    return rewired_network
...
def generate_shuffled_network(ggi_network, seed):
    ...
    shuffled_gene_ids = list(
        nx.get_node_attributes(shuffled_network, "GeneID").values()
    )
    np.random.seed(seed=seed)
    np.random.shuffle(shuffled_gene_ids)
    shuffled_gene_ids = {
        node: shuffled_gene_ids[node] for node in shuffled_network.nodes()
    }
...
def generate_scale_free_network(ggi_network, seed):
    ...
    m = np.max(
        [1, round(num_nodes / 2.0 - np.sqrt((num_nodes * num_nodes) / 4.0 - num_edges))]
    )
